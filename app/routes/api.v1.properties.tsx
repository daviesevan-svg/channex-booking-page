...
import { guestDefaultLang } from "~/lib/content";
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  // No ?lang = the property's guest default (which need not be English).
  const settings = await getSettings(auth.pid);
  const lang = new URL(request.url).searchParams.get("lang") ?? guestDefaultLang(settings);
  const ov = await getOverrides(auth.pid, lang);
