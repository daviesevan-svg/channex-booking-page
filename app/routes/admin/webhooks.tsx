...
export async function action({ request }: Route.ActionArgs) {
  ...
  if (!(await canManageProperty(request, propertyId))) return { error: "Only an owner or manager can manage webhooks." };
...
export default function AdminWebhooks({ loaderData, actionData }: Route.ComponentProps) {
  ...
  if (!loaderData.configured) {
    return (
      <div className="rounded-[14px] border border-line bg-surface p-6">
        <h1 className="mb-2 font-serif text-[22px] font-semibold">{t("whTitle")}</h1>
        <p className="text-[15px] text-secondary">{t("whNotConfigured")}</p>
      </div>
    );
  }
  if (!loaderData.canManage) {
    return (
      <div className="rounded-[14px] border border-line bg-surface p-6">
        <h1 className="mb-2 font-serif text-[22px] font-semibold">{t("whTitle")}</h1>
        <p className="text-[15px] text-secondary">{t("whOnlyOwner")}</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <h1 className="mb-1 font-serif text-[26px] font-semibold">{t("whTitle")}</h1>
      <p className="mb-5 max-w-2xl text-[14px] text-secondary">
        ...
        {t("whIntro2")} <code className="rounded bg-line/40 px-1">&lt;ts&gt;.&lt;body&gt;</code>{t("whIntro3")}
      </p>
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="border-b border-divider text-left text-[12px] uppercase tracking-wide text-muted">
                ...
                <th className="px-5 py-3 font-semibold">{t("whEvents")}</th>
                <th className="px-5 py-3 font-semibold">{t("whSecret")}</th>
              ...
              {endpoints.map((e) => (
                <tr key={e.id} className="border-b border-divider/60 last:border-0 align-top">
                  ...
                  <td className="px-5 py-3 text-right">
                    <Form method="post" onSubmit={(ev) => { if (!confirm(t("whRemoveConfirm"))) ev.preventDefault(); }}>
                      <input type="hidden" name="intent" value="remove" />
                      <input type="hidden" name="id" value={e.id} />
                      <button type="submit" disabled={busy} className="text-[13px] font-semibold text-red-600 hover:underline disabled:opacity-60">{t("whRemove")}</button>
                    </Form>
                  </td>
                </tr>
              ))}
