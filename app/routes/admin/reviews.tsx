  ...
  if (!loaderData.configured) {
    return (
      <div className="rounded-[14px] border border-line bg-surface p-6">
        <h1 className="mb-2 font-serif text-[22px] font-semibold">{t("rvTitle")}</h1>
        <p className="text-[15px] text-secondary">{t("rvAddPropertyFirst")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-baseline justify-between">
        <h1 className="font-serif text-[26px] font-semibold">{t("rvTitle")}</h1>
        {average != null && (
          <span className="text-[14px] text-secondary">
            <span className="font-serif text-[22px] font-semibold text-ink">{average}</span> / 5 ·{" "}
            {t(reviews.length === 1 ? "rvReviewsCount_one" : "rvReviewsCount_other", { n: reviews.length })}
          </span>
        )}
      </div>
