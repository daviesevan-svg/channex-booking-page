...
export default function AdminUsers({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div>
      <h1 className="mb-1 font-serif text-[26px] font-semibold">Users</h1>
      <p className="mb-6 text-[14px] text-muted">
        Anyone who signs in gets an account and can add their own properties. Superadmins see every
        property and manage users here. Sign-in is open — restrict it with{" "}
        <code className="rounded bg-chip px-1 py-0.5 text-[12px]">ADMIN_EMAILS</code>.
      </p>

      <div className="overflow-hidden rounded-[14px] border border-line bg-surface">
        {rows.length === 0 && (
          <div className="px-5 py-6 text-[14px] text-muted">No users yet.</div>
        )}
        {rows.map((u, i) => (
          <div
            key={u.email}
              ...
              <div className="flex items-center gap-2.5">
                ...
                {u.superadmin && (
                  <span className="rounded-full bg-[#ece6f0] px-2 py-0.5 text-[11px] font-semibold text-[#6b4f8a]">
                    Superadmin{u.envLocked ? " · env" : ""}
                  </span>
                )}
              </div>
              <div className="mt-0.5 text-[12px] text-muted-2">
                {u.properties} {u.properties === 1 ? "property" : "properties"} · joined{" "}
                {fmtDate(u.createdAt)}
              </div>
            </div>
              ...
              {u.email !== me && !u.envLocked ? (
                <Form method="post">
                  ...
                  <button type="submit" disabled={busy} className="text-accent hover:underline disabled:opacity-50">
                    {u.superadmin ? "Make member" : "Make superadmin"}
                  </button>
                </Form>
              ) : (
                <span className="text-faint">role locked</span>
              )}
              ...
              {u.email !== me && !u.envLocked && (
                <Form
                  ...
                  onSubmit={(e) => {
                    if (
                      !confirm(
                        `Remove ${u.email}? Their properties are kept but become unassigned for you to reassign.`,
                      )
                    )
                      e.preventDefault();
                  }}
