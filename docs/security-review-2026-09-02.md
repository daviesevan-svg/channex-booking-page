...
**Status: fixed in the follow-up PR `viva-refund-hardening` (2026-09-02).** `rejectMismatchedVivaPayment` in `booking-finalize.server.ts` refunds the transaction (behind a once-only `refund_claim` latch so the return URL and the webhook can't both refund) and deletes the pending; the guest lands back on checkout with a "refunded, nothing booked" notice in all ten guest languages. The currency-lock-while-connected recommendation below is still open.
...
**Status: fixed in the same follow-up PR.** `refundBookingCharge` now takes a permanent D1 `refund_claim` (`refund-claim.server.ts`) before calling either gateway and releases it only on gateway failure; `cancelBookingIfActive` (`bookings.server.ts`) makes the active→cancelled transition a conditional `UPDATE … WHERE lifecycle='active'` so exactly one of two racing cancels runs the inventory/Channex/refund/email side effects — used by both the guest and the admin cancel; the guest `cancel` intent is throttled 5 / 10 min per client. Pinned by `refund-once.test.ts` against real SQLite.
...
**Status: fixed in the follow-up PR `image-gc-scope-and-manage-ratelimit` (2026-09-02).** `image-paths.ts` defines the property-upload roots; `deleteUnreferencedImages` now only ever deletes keys under the calling property's own `<root>/<pid>/` prefixes (partner assets, feed snapshots, other properties' and legacy keys are skipped — a leak, never a deletion), and the management API validators accept only property-upload paths as image references. Pinned by `image-gc-scope.test.ts`.
...
**Status: fixed in the same follow-up PR.** `authenticateApiKey` enforces 300 reads / 60 writes per 10 min per manage key (429); invite requests 5/hour per property; image upload + import 20 / 10 min per key; `/mcp` 300 POSTs / 10 min per client with batches capped at 20. `portal.auto_refund` is refused by `validatePropertyPatch` with a pointer to the admin (still readable). Pinned by `manage-rate-limit.test.ts`.
