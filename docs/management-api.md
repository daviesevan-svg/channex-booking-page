...
|---|---|---|---|
| Property settings | `GET/PATCH /v1/manage/property` | `getSettings` / `patchSettings` + the narrow savers | PATCH allowlist: identity, address+lat/lng, check-in/out times, currency, languages + `default_language` (the guest default, one of the enabled languages; 2026-09-09), facilities (curated keys), pricingMode, timezone, cutoffs, portal policy (except `auto_refund` — owner-only in the admin, readable but not writable here since 2026-09-02). `portal.after_deadline_message` is the DEFAULT-language text (it lives in settings); its per-language translations are edited in the admin's Portal page under the editing-language switcher and have no API surface yet, singleUnit. NOT writable: `connectedSystem` (the Channex gate — flipping it enables live traffic; superadmin UI only for now), `stripeAccountId`/`stripeChargesEnabled`, `websiteDomain` (order-sensitive claim/release flow), `liveBooking`. `websiteEnabled` IS writable (2026-08-28, after the MCP dogfood): it is content-safe by construction — the website layer renders or not, nothing is destroyed either way. |
...
| Rate plans (structural) | same shape as rooms, `PUT` via `replaceRates` | `catalog.server.ts`, `rate-policy.ts`, `rate-pricing.ts` | Exposes title, mealPlan, `prices` (base price per room — structural, NOT the ARI grid), occupancyPricing, structured `policy`, inclusions, active. `policy.cancellation.tiers` is a schedule, validated as one (docs/cancellation-tiers.md §3.5): most generous first, each deadline closer to arrival, each penalty at least as harsh, at most 4 — a schedule is either right everywhere or refused here. **`channexRateIds` is server-owned and never writable** — every write preserves it (ARI and booking pushes key on it). |
...
| Extras | CRUD + `PUT` replace | `extras.server.ts` | `taxable` default-true documented in the schema. `max_qty` (integer ≥ 1, null = no limit) caps the guest quantity; `1` = charged exactly once and the guest page shows no quantity control — `resolveExtras` clamps and `POST /v1/bookings` 422s above it. The demo-seed (`ensureExampleExtras`) must NOT run on API list — API callers get the real (possibly empty) list. |
...
| Branding/theme | `PATCH /v1/manage/brand` | `saveBrand` / `saveThemeTokens` | `themeFont` restricted to `FONT_PAIRS` ids; invalid values are 422, not silently ignored. `custom_css` is the property's own guest-page stylesheet (≤ 20 000 chars, `<` escaped and `@import` dropped on save — `app/lib/custom-css.ts`; vocabulary in `docs/custom-css.md`). `GET /v1/manage/brand-kit` returns the derived kit (prompt + tokens.json) — pure read, useful to agents building a matching site. |
